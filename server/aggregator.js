...
Aggregator.prototype.startPolling = function (resource) {
  ...
  log.info('Scheduling (' + resource.id + ',' + resource.url + ',' + resource.interval + ')');
...
Aggregator.prototype.scheduleAnotherIteration = function (resource) {
  ...
  log.info('Rescheduling (' + resource.id + ',' + resource.url + ',' + resource.interval + ')');
...
Aggregator.prototype.stopPolling = function (resource) {
  log.info('Stopping (' + resource.id + ',' + resource.url + ')');
...
function emitResponse (resource, error, response, body) {
  ...
  if(error) {
    log.info('[' + timeDiff + 'ms] Error (' + resource.id + ',' + resource.url + ')');
...
function onSocketData (message) {
  try {
    ...
    switch(message.action) {
      case 'poll-start':
        log.info ('Poll start (' + r.method + ',' + r.id + ',' + r.url + ')');
        this.startPolling(r);
        break;
      case 'request':
        log.info ('Single request (' + r.method + ',' + r.id + ',' + r.url + ')');
        r.startTs = Date.now();
        request(r, emitResponse.bind(this, r));
        break;
      case 'poll-stop':
        log.info ('Poll stop (' + r.id + ',' + r.url + ')');
        this.stopPolling(r);
        break;
    }
  }
